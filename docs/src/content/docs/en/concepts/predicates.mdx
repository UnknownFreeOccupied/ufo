...
| Predicate | Arguments | Description |
| --- | --- | --- |
| `Satisfies` | `RetFun`, `IntFun` | `node` satisfies `RetFun` and the node's ancestors satisfy `IntFun`. |
...
| Predicate     | Arguments    | Description                                             |
| ------------- | ------------ | ------------------------------------------------------- |
| `Alpha`       | `min`, `max` | `node`'s alpha is within the range `[min, max]`.        |
| `Hue`         | `min`, `max` | `node`'s hue is within the range `[min, max]`.          |
| `Saturation`  | `min`, `max` | `node`'s saturation is within the range `[min, max]`.   |
| `Lightness`   | `min`, `max` | `node`'s lightness is within the range `[min, max]`.    |
| `HasColor`    |              | `node` has any color.                                   |
| `ColorWeight` | `min`, `max` | `node`'s color weight is within the range `[min, max]`. |

### Confidence

| Predicate     | Arguments    | Description                                             |
| ------------- | ------------ | ------------------------------------------------------- |
| `Confidence`  | `min`, `max` | `node`'s confidence is within the range `[min, max]`.    |

### Cost

| Predicate     | Arguments    | Description                                             |
| ------------- | ------------ | ------------------------------------------------------- |
| `Cost`        | `min`, `max` | `node`'s cost is within the range `[min, max]`.          |

### Count

| Predicate     | Arguments    | Description                                             |
| ------------- | ------------ | ------------------------------------------------------- |
| `Count`       | `min`, `max` | `node`'s count is within the range `[min, max]`.         |

### Distance

| Predicate     | Arguments    | Description                                             |
| ------------- | ------------ | ------------------------------------------------------- |
| `Distance`    | `min`, `max` | `node`'s distance is within the range `[min, max]`.      |

### Intensity

| Predicate     | Arguments    | Description                                             |
| ------------- | ------------ | ------------------------------------------------------- |
| `Intensity`   | `min`, `max` | `node`'s intensity is within the range `[min, max]`.    |

### Label

| Predicate     | Arguments    | Description                                             |
| ------------- | ------------ | ------------------------------------------------------- |
| `Label`       | `label`      | `node` has the given `label`.                           |
...
| ------------------ | ------------ | ------------------------------------------------------- |
| `Free`             |              | `node` is free.                                         |
| `Occupied`         |              | `node` is occupied.                                     |
| `Unknown`          |              | `node` is unknown.                                      |
| `Occupancy`        | `min`, `max` | `node`'s occupancy is within the range `[min, max]`.    |
| `OccupancyStates`  | `states`     | `node`'s occupancy is one of the given `states`.         |
| `ContainsFree`     |              | `node` contains free space (i.e., either it is free or has free children). |
| `ContainsOccupied` |              | `node` contains occupied space (i.e., either it is occupied or has occupied children). |
| `ContainsUnknown`  |              | `node` contains unknown space (i.e., either it is unknown or has unknown children). |

### Reflection

| Predicate          | Arguments    | Description                                             |
| ------------------ | ------------ | ------------------------------------------------------- |
| `Hits`             | `min`, `max` | `node` has been hit within the range `[min, max]`.      |
| `Misses`           | `min`, `max` | `node` has been missed within the range `[min, max]`.   |
| `Reflectiveness`   | `min`, `max` | `node`'s reflectiveness is within the range `[min, max]`. |
...
| Predicate  | Arguments    | Description                                                    |
| ---------- | ------------ | -------------------------------------------------------------- |
| `Radius`   | `min`, `max` | `node`'s surfel radius is within the range `[min, max]`.       |
| `IsPlanar` |              | `node`'s surfel represents a flat, planar surface.             |

### Time

| Predicate          | Arguments    | Description                                          |
| ------------------ | ------------ | ---------------------------------------------------- |
| `Time`             | `min`, `max` | `node`'s timestamp is within the range `[min, max]`. |

### Void Region

| Predicate            | Description                                          |
| -------------------- | ---------------------------------------------------- |
| `VoidRegion`         | `node` is a void region.                             |
| `ContainsVoidRegion` | `node` contains a void region (i.e., either itself or any of its children is a void region). |
